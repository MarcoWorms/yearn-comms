  ...
  return (
    <div>
      {(allPosts || []).map((post, index) => {
        if (!post || !post?.slug) {
          return (
            <div className={'mb-4 break-inside'} key={`${index}${language}`}>
              <div
                className={
                  'overflow-hidden w-full bg-white dark:bg-black rounded-sm cursor-pointer'
                }
              ></div>
            </div>
          )
        ...
        return (
          <div
            className={'pb-6 break-inside'}
            key={`${post.path}${post.slug}${index}${language}`}
          >
            <Link href={`/${post.path}${post.slug}`}>
              <div
                className={
                  'w-full bg-white dark:bg-black rounded-sm shadow-none hover:shadow-sm transition-shadow cursor-pointer'
                }
              >
                <div
                  className={
                    'flex w-full border-b border-gray-blue-3 dark:border-gray-2'
                  }
                >
                  <Image
                ...
                <div className={'p-4'}>
                  <div className={'flex flex-row justify-between mb-2 w-full'}>
                    <p
                      className={'text-xs text-gray-blue-1 dark:text-gray-3'}
                    >{`by ${post?.author || 'Yearn'}`}</p>
                  ...
                  <h2
                    className={
                      'font-title text-2xl font-bold text-dark-blue-1 dark:text-white'
                    }
                    dangerouslySetInnerHTML={{
                      __html: parseMarkdownUnset(post?.title || ''),
                    }}
                  />
                </div>
              </div>
